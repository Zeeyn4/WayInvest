...
export async function sendVerificationCode(to: string, code: string) {
  ...
  const res = await fetch('https://api.brevo.com/v3/smtp/email', {
    ...
    body: JSON.stringify({
      sender: { name: 'LamInvest', email: SENDER_EMAIL },
      to: [{ email: to }],
      subject: `${code} — код подтверждения LamInvest`,
      htmlContent: `
        <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;padding:32px;background:#111318;color:#E8E4DC;border-radius:16px;">
          <h1 style="color:#C9A84C;font-size:24px;margin-bottom:8px;">LamInvest</h1>
          <p style="color:#8A8680;font-size:14px;margin-bottom:24px;">Первая инвест-платформа регионального рынка</p>
